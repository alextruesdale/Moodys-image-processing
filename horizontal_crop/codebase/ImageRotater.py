...
class ImageRotater(object):
    ...
    def __init__(self, file):
        ...
        self.rotation_data = self.rotate_on_curve()
        self.threshold = self.rotation_data[1]
        self.max_point = self.rotation_data[0]
    ...
    def rotate_check(self, x_value, anchor_points, threshold):
    ...
    def rotate_on_curve(self):
        ...
        points = list(np.linspace(359.2, 361.7, 7))
        ...
        while straightened == 0:
            anchor_points = []
            for point in points:
                self.rotate_check(point, anchor_points, threshold)

            found_whiteness = [anchor_point[1] for anchor_point in anchor_points]

            if any(value > 400 for value in found_whiteness):
                break
            else:
                threshold -= .01

        # print('gutter threshold:', threshold)
        straightened == 1
