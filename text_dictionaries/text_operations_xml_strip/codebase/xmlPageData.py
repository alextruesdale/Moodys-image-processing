...
class xmlPageData(object):
    ...
    def define_page_content_bounds(self):
        ...
        return (page_bounds_final, list_list)
    ...
    def page_data(self):
        ...
        left_column_start = sorted([value for value in self.left_list if value > center - .005])
        third_one_start = sorted([value for value in self.left_list if third_first > center - .005])
        third_two_start = sorted([value for value in self.left_list if third_second > center - .005])

        if len(left_column_start) > 0:
            left_column_start = left_column_start[0]
        elif len(left_column_start) == 0 and len(self.left_list) > 0:
            left_column_start = self.left_list[0]
        else:
            left_column_start = 0
