...
class ImageSaver(object):
    ...
    def __init__(self, file, image_input, image_meta_data):
    ...
    def image_save(self):
        ...
        for image in self.output_images[2]:
            count = str(i).zfill(2)
            Image.fromarray(image).save(os.path.join(save_folder, os.path.basename(self.file)[:-4] + '_vertical_cut_' + count + '.tif'))
            i += 1

        # Save vertical cropped images.
        i = 1
        if len(self.output_images) > 3:
            for image in self.output_images[3]:
                count = str(i).zfill(2)
