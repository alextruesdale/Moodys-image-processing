...
def main():
    ...
    for file in files:
        ...
        print(file)
        try:
            ImageOperator.ImageOperator(file, files_operated)
    ...
    jobdata = []
    job_time = 'job time: ' + str(round(job_time, 2)) + ' seconds ' + '... ' + str(round((job_time / 60), 3)) + ' minutes'
    ...
    newline = '\n'
    with open('output/job_data.txt', mode='wt', encoding='utf-8') as job_data:
