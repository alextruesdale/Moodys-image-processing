    ...
    def image_save(self):
        ...
        if len(self.output_images) > 2:
            for image in self.output_images[2:]:
                ...
                if not os.path.exists(os.path.join(save_folder, cut)):
                    os.mkdir(os.path.join(save_folder, cut))

                Image.fromarray(image).save(os.path.join(save_folder, cut, 'vertical_cut_' +
                                                         os.path.basename(self.file)))
                i += 1
